...
**Every build target builds it already.** `make native`, `make win32`, `make everything` and the rest build the botlib out of `vendor/gladiator-bot-restored/` for the target whose library they have just built, and leave it in that same build directory under the name the game `dlopen`s - `release-win32/gladiator.dll`, `release/gladiator.so`, and so on for each. `GLAD_SERVERFIX=1` is the default here; `GLAD_SERVERFIX=0` asks for the submodule's own default instead, and `GLADDIR=<path>` builds from another checkout.

It is still a separate repository and a separate build - nothing in `src/` includes a line of it, and `docs/botlib-contract.md` is the only thing that crosses. **A tree whose submodule is not checked out still builds and still passes the audits:** the botlib step says it is skipping, and the library is built anyway. That property is what lets `ci.yml`'s server-checks job check this tree out without submodules at all.

Two things follow from the submodule having one build directory and one output path, with no target named in either. Switching target would hand the next link the last one's objects - measured, and it fails as `build/botlib_debug.o: file format not recognized` - so the build stamps what that tree currently holds and cleans when it is about to change. And five targets under `make -j` would drive that one directory at once, so the step takes a `mkdir` lock around it; two targets alternating rebuild the botlib each time they trade places.

To build it by hand - for a platform no target covers, or to work on the botlib itself - the submodule's own Makefile is unchanged:
...
**`GLAD_SERVERFIX=1` is that flag, and a package and a server are both built with it.** The submodule is a reconstruction of the 1999 binaries, so its default build reproduces them bugs and all -- the build its ASM-matching oracles measure, and the right default there. The gate builds out the original bugs that are fatal or simply wrong in play -- twelve at the pinned commit. Seven are in this object, and two of those are crashes a real map reaches -- an unbounded push into an `int[64]` whose 65th slot is the return address, and a routing seed that reads past its own allocation when the goal area has no reachabilities; the rest range from a bot navigating to one item through another's bounding box to a moved bot keeping its old client number, and one (the zipped-AAS working-directory fix) reaches the object on Windows only. The remaining five are in the submodule's own game tree, which this project does not build. So `.github/workflows/release.yml` and `server/Dockerfile` both pass it, and `package.sh` reads the staged botlib back for a string only the gated arm can carry -- meaning a local packaging rehearsal needs the flag too, or it stops at that check.

Independently of the gate, the pin carries the 64-bit port-correctness fixes that ship in every 64-bit build whatever `GLAD_SERVERFIX` is set to: a Windows-x64 crash loading a map with no `.aas`, a team-chat checkpoint lookup that read a truncated pointer, a character leaked on every bot removal, and a bot moved between client slots losing its side-band character and weapon state. The 32-bit builds were never affected by any of these.
