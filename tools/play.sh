...
RULESET=dm MAP=q2dm1 NAME=TESTBOT PORT=0 SRVARGS= CLARGS= TMO=90
SCRIPT=${1:-}; shift || true
while getopts "r:m:n:p:s:c:t:" o; do
  case $o in
...
( ulimit -c 0
  timeout -s KILL "$TMO" xvfb-run -a -s "-screen 0 640x480x24" \
    ...
    +connect "localhost:$PORT" >"$OUT/client.stdout" 2>&1 )
CLRC=$?
...
echo "=== play.sh: ruleset=$RULESET map=$MAP port=$PORT client_rc=$CLRC verdict=$rc ==="
exit $rc
