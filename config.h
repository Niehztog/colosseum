...
// GAME_API_VERSION_NEW (3302) with gclient_new_t / pmove_new_t, and MAX_STATS
// is 64 (MAX_STATS_NEW) rather than 32, which the stat slot map depends on.
// `make API=old` overrides it to 0 for a GAME_API_VERSION_OLD (3) library.
//
// USE_PROTOCOL_EXTENSIONS is on so game.csr can be the extended configstring
// remap; whether it is actually used is negotiated at runtime against
// sv_features, not decided here.
//
// VERSION is "colosseum", replacing the donors' "osp-q2pro" and "ra2-q2pro".
// CPUSTRING is derived from the build by the Makefile rather than hardcoded
// "x86" as both donors leave it.
...
// The one switch in this file the Makefile may override, so that
...
// on the command line plus a bare #define here is a redefinition, which is a
// build failure rather than a warning.  New is the default and the
