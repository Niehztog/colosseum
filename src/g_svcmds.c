...
void    ServerCommand(void)
{
    ...
    if (Q_stricmp(cmd, "ruleset") == 0)
        ...
        SVCmd_WriteIP_f();
    // Tourney's five `sv` commands, delegated like its client commands: one
    ...
    // it cannot shadow a name this file already owns.
        ...
        ;
    // The SDK's own set, `server` true because this is `sv <cmd>`.
    ...
    // present in four rulesets -- cannot shadow a ruleset's own name.
        ...
        ;
    // `openlog`, `closelog`, `writelog`.  The donor asked these in
