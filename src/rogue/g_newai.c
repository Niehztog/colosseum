...
bool monsterlost_checkhint(edict_t *self)
{
    ...
    // ROGUE declared count1/count2/count4 here and never read them -- they fed
    // the gi.dprintf()s it left commented out.  count5 IS read.  No behaviour.
    int         count5 = 0;
    float       r;
    int         i;
...
{
    edict_t     *e, *current;
    // ditto: count2 was a diagnostic counter with no reader.
    int         field, i;
