    ...
    if (!current_bad && new_bad) {
        if (new_bad->owner) {
            ...
            if (!strcmp(new_bad->owner->classname, "tesla")) {
                ...
                // R-187, and this file is NOT BUILT (R-MP-5) -- fixed anyway,
                // because a reference copy that disagrees with the built one is
                // a trap for whoever reads it next.  See src/m_move.c.
