...
int DBall_CheckDMRules(void)
{
    if (goallimit && goallimit->value) {
        ...
        // Through the dispatch, like every other rules row that ends a level.
        // A `DMGame` row is not a `ruleset_ops_t` row, so `EndDMLevel()` here
        // is not a same-row fallback -- it is baseq2's half reached by name,
        // and it would take `sv_maplist` while `arena` had RA2's `maploop:` in
        // `ops_arena.EndLevel` and tourney had `osp_maps.c` in its own.
        // `G_EndLevel()` is the same call `CheckDMRules()` makes one frame
        // above this one, which is the relationship these two stand in.
        G_EndLevel();
        return 1;
    }
