...
// Ctf_hook selects the offhand hook (the 1999 default: on);
// laserhook selects the beam rendering and is what the `laserhook` libvar
// reports to the brain.
cvar_t *ctf_hook;
cvar_t *laserhook;
...
// Threewave's complete 130-item `ctf_statusbar` literal was here.  The bar is
// now emitted at runtime from the active ruleset's slot map by
// g_stats.c's sb_ctf_tail(), item for item and in this order.  A literal is
// what made slots 17/18/19 unresolvable in the first place.
...
void CTFSpawn(void)
{
    ...
    // Everything above runs in every ruleset, so that flag1_item and flag2_item
    // are resolved for the shared drop paths -- player_die and ClientDisconnect
    // call CTFDeadDropFlag with no gate, and it indexes the inventory by
    // ITEM_INDEX(flag1_item).
    //
    // Everything below is CTF's alone.  The tech spawner is an edict with a
    // think, so leaving it ungated put a Disruptor Shield in single player and
    // one extra edict in every map -- caught by `sv ruleset`'s census reading
    // 392 where the same map had read 391.  And a server with `competition 2`
    // left in its config would have started a *deathmatch* in MATCH_SETUP,
    // where CTFMatchSetup() freezes all damage.
...
void CTFInit(void)
{
    ...
    // `ctf` is registered and reconciled by the ruleset dispatch:
...
void CTFPrecache(void)
{
    ...
    // caught that one, because a precache is not an entity.
    //
    // The grapple is the one row that is not ctf's alone: RA2's fork was
    // dropped and RA_HookThink() fires Threewave's hook, while
...
// The client being placed is threaded through all three; under `ctf` the
...
void CTFAssignSkin(edict_t *ent, char *s)
{
    ...
    // thing that needs to know.  Gladiator's g_ctf.c adds the userinfo write
    // at all three arms under `#ifdef BOT` -- the same fence that hid
    // `ctfteam` -- and the reason is `bl_main.c`: the brain is told a
    ...
    // which is bots shooting their own side.  The identical question is
    // answered for RA2's teams elsewhere; this is CTF's, and unlike RA2's it
    // is the donor's own answer rather than a new one.
...
The same argument a second ruleset over.  Threewave declares no
...
is the obvious thing to try -- the same trap as a non-pickup arena.  Threewave's mappers put spawn points in a base for variety: `q2ctf1`
...
the sides are level take one off either.  RA_ArenaBotName is the same
...
void CTFDropFlagTouch(edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
    ...
    // compared against a bare literal, which is a third form.
    if (other == ent->owner &&
...
static void CTFSetIDView(edict_t *ent)
{
    ...
    G_SetStat(ent, SID_CTF_ID_VIEW, 0);
    G_SetStat(ent, SID_CTF_ID_VIEW_COLOR, 0);
    ...
    if (tr.fraction < 1 && tr.ent && tr.ent->client) {
        G_SetStat(ent, SID_CTF_ID_VIEW, game.csr.general + (tr.ent - g_edicts - 1));
        if (tr.ent->client->resp.ctf_team == CTF_TEAM1)
            G_SetStat(ent, SID_CTF_ID_VIEW_COLOR, imageindex_sbfctf1);
        else if (tr.ent->client->resp.ctf_team == CTF_TEAM2)
            G_SetStat(ent, SID_CTF_ID_VIEW_COLOR, imageindex_sbfctf2);
        return;
    }
    ...
    if (bd > 0.90f) {
        G_SetStat(ent, SID_CTF_ID_VIEW, game.csr.general + (best - g_edicts - 1));
...
void SetCTFStats(edict_t *ent)
{
    ...
    if (ctfgame.match > MATCH_NONE)
        G_SetStat(ent, SID_CTF_MATCH, CONFIG_CTF_MATCH);
    else
        G_SetStat(ent, SID_CTF_MATCH, 0);

    if (ctfgame.warnactive)
        G_SetStat(ent, SID_CTF_TEAMINFO, CONFIG_CTF_TEAMINFO);
    else
        G_SetStat(ent, SID_CTF_TEAMINFO, 0);
    ...
    G_SetStat(ent, SID_CTF_TEAM1_HEADER, imageindex_ctfsb1);
    G_SetStat(ent, SID_CTF_TEAM2_HEADER, imageindex_ctfsb2);

    // if during intermission, we must blink the team header of the winning team
    if (level.intermission_framenum && (level.framenum & 8)) { // blink 1/8th second
        // note that ctfgame.total[12] is set when we go to intermission
        if (ctfgame.team1 > ctfgame.team2)
            G_SetStat(ent, SID_CTF_TEAM1_HEADER, 0);
        else if (ctfgame.team2 > ctfgame.team1)
            G_SetStat(ent, SID_CTF_TEAM2_HEADER, 0);
    ...
    // tech icon
    i = 0;
    G_SetStat(ent, SID_CTF_TECH, 0);
    while (tnames[i]) {
        if ((tech = FindItemByClassname(tnames[i])) != NULL &&
            ent->client->pers.inventory[ITEM_INDEX(tech)]) {
            G_SetStat(ent, SID_CTF_TECH, gi.imageindex(tech->icon));
            break;
        }
        i++;
    }
    ...
    G_SetStat(ent, SID_CTF_TEAM1_PIC, p1);
    G_SetStat(ent, SID_CTF_TEAM2_PIC, p2);
    ...
    if (ctfgame.last_flag_capture && level.time - ctfgame.last_flag_capture < 5) {
        if (ctfgame.last_capture_team == CTF_TEAM1)
            if (level.framenum & 8)
                ...
                G_SetStat(ent, SID_CTF_TEAM1_PIC, 0);
        else if (level.framenum & 8)
            G_SetStat(ent, SID_CTF_TEAM2_PIC, p2);
        else
            G_SetStat(ent, SID_CTF_TEAM2_PIC, 0);
    }

    G_SetStat(ent, SID_CTF_TEAM1_CAPS, ctfgame.team1);
    G_SetStat(ent, SID_CTF_TEAM2_CAPS, ctfgame.team2);

    G_SetStat(ent, SID_CTF_FLAG_PIC, 0);
    if (ent->client->resp.ctf_team == CTF_TEAM1 &&
        ent->client->pers.inventory[ITEM_INDEX(flag2_item)] &&
        (level.framenum & 8))
        G_SetStat(ent, SID_CTF_FLAG_PIC, imageindex_i_ctf2);

    else if (ent->client->resp.ctf_team == CTF_TEAM2 &&
             ent->client->pers.inventory[ITEM_INDEX(flag1_item)] &&
             (level.framenum & 8))
        G_SetStat(ent, SID_CTF_FLAG_PIC, imageindex_i_ctf1);

    G_SetStat(ent, SID_CTF_JOINED_TEAM1_PIC, 0);
    G_SetStat(ent, SID_CTF_JOINED_TEAM2_PIC, 0);
    if (ent->client->resp.ctf_team == CTF_TEAM1)
        G_SetStat(ent, SID_CTF_JOINED_TEAM1_PIC, imageindex_i_ctfj);
    else if (ent->client->resp.ctf_team == CTF_TEAM2)
        G_SetStat(ent, SID_CTF_JOINED_TEAM2_PIC, imageindex_i_ctfj);
...
//  end[2] = self->absmin[2] + self->size[2] / 2;
    ...
    // The laserhook.  Threewave shipped both renderings and chose between
    // them with `#if 1 //def USE_GRAPPLE_CABLE`; the choice is a runtime cvar
    // here because the brain has to be told which one is in play (the
    // `laserhook` libvar) and a compile-time switch cannot be reported.
    gi.WriteByte(svc_temp_entity);
    if (laserhook->value) {
...
{
    // The offhand hook releases from `hookoff` rather than from letting go of
    // the attack button, so its release is handled here rather than in
    // CTFWeapon_Grapple.
    ...
    // the same defect in the second of the two functions that has it, and the
    ...
    // the whole test is false and the hook stays out.
...
// The offhand hook, as 1999 shipped it.  Fired from ClientThink rather
// than from a weapon think, so it costs no weapon slot and does not interrupt
// whatever the player is holding.  The FIRED bit is what makes `+hook` a hold
// rather than a machine gun: one grapple per press.
void CTFHook_Fire(edict_t *ent)
{
    if (ent->client->ctf_hookstate & CTF_HOOK_STATE_FIRED)
        return;
    CTFGrappleFire(ent, vec3_origin, 10, 0);
    ent->client->ctf_hookstate |= CTF_HOOK_STATE_FIRED;
}

// Neither an observer nor a corpse fires it.
//
// 1999's guard asks `ent->solid != SOLID_NOT`, and the note that used to stand here
...
// rather than playing" has three spellings in this library, and
// `deadflag` because being dead has one and the donor's proxy for it was
// wrong.
...
// the first.  CTFHookAllowed() above is the one this file needed.
...
void CTFScoreboardMessage(edict_t *ent, edict_t *killer)
{
    ...
    *string = 0;
    ...
    // team one
    // The two team headers and the two capture counts, by slot id rather than by
    // literal number.  The slot map is not only about the statusbar: a layout reads
    // stats[] by number too, and Threewave wrote 18, 20, 24 and 25 in here as
    // literals -- a second place where CTF's numbering lived, and the one the
    // slot audit would have kept reporting.
    Q_snprintf(string, maxsize,
               "if %d xv 8 yv 8 pic %d endif "
               "xv 40 yv 28 string \"%4d/%-3d\" "
               "xv 98 yv 12 num 2 %d "
               "if %d xv 168 yv 8 pic %d endif "
               "xv 200 yv 28 string \"%4d/%-3d\" "
               "xv 256 yv 12 num 2 %d ",
               G_Stat(SID_CTF_TEAM1_HEADER), G_Stat(SID_CTF_TEAM1_HEADER),
               totalscore[0], total[0],
               G_Stat(SID_CTF_TEAM1_CAPS),
               G_Stat(SID_CTF_TEAM2_HEADER), G_Stat(SID_CTF_TEAM2_HEADER),
               totalscore[1], total[1],
               G_Stat(SID_CTF_TEAM2_CAPS));
    len = strlen(string);
...
struct {
    ...
    {   "item_armor_combat",        6 },
    {   "item_armor_jacket",        6 },
    // The content layers' weapons, at the same priority Threewave gives
    // baseq2's, so `say_team %l` can say "near the Ion Ripper" on a map that has
    // one.  Both content layers are valid under ctf, and this table decides
...
{
    // 1024, as Threewave had them.  Shrinking these to 256 while
    // CTFSay_Team_Sight still composes into s[1024] and ends with
    // strcpy(buf, s) is a client-reachable stack overflow via `say_team %n`
    // on a server with enough visible players.
    char outmsg[1024];
    char buf[1024];
    int i;
    char *p;
...
static void SetLevelName(ctf_pmenu_t *p)
{
...
void CTFJoinTeam1(edict_t *ent, ctf_pmenuhnd_t *p);
void CTFJoinTeam2(edict_t *ent, ctf_pmenuhnd_t *p);
void CTFCredits(edict_t *ent, ctf_pmenuhnd_t *p);
void CTFReturnToMain(edict_t *ent, ctf_pmenuhnd_t *p);
void CTFChaseCam(edict_t *ent, ctf_pmenuhnd_t *p);

ctf_pmenu_t creditsmenu[] = {
    { "*Quake II",                      CTF_PMENU_ALIGN_CENTER, NULL },
    { "*ThreeWave Capture the Flag",    CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,                             CTF_PMENU_ALIGN_CENTER, NULL },
    { "*Programming",                   CTF_PMENU_ALIGN_CENTER, NULL },
    { "Dave 'Zoid' Kirsch",             CTF_PMENU_ALIGN_CENTER, NULL },
    { "*Level Design",                  CTF_PMENU_ALIGN_CENTER, NULL },
    { "Christian Antkow",               CTF_PMENU_ALIGN_CENTER, NULL },
    { "Tim Willits",                    CTF_PMENU_ALIGN_CENTER, NULL },
    { "Dave 'Zoid' Kirsch",             CTF_PMENU_ALIGN_CENTER, NULL },
    { "*Art",                           CTF_PMENU_ALIGN_CENTER, NULL },
    { "Adrian Carmack Paul Steed",      CTF_PMENU_ALIGN_CENTER, NULL },
    { "Kevin Cloud",                    CTF_PMENU_ALIGN_CENTER, NULL },
    { "*Sound",                         CTF_PMENU_ALIGN_CENTER, NULL },
    { "Tom 'Bjorn' Klok",               CTF_PMENU_ALIGN_CENTER, NULL },
    { "*Original CTF Art Design",       CTF_PMENU_ALIGN_CENTER, NULL },
    { "Brian 'Whaleboy' Cozzens",       CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,                             CTF_PMENU_ALIGN_CENTER, NULL },
    { "Return to Main Menu",            CTF_PMENU_ALIGN_LEFT, CTFReturnToMain }
};
...
ctf_pmenu_t joinmenu[] = {
    { "*Quake II",          CTF_PMENU_ALIGN_CENTER, NULL },
    { "*ThreeWave Capture the Flag",    CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,                 CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,                 CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,                 CTF_PMENU_ALIGN_CENTER, NULL },
    { "Join Red Team",      CTF_PMENU_ALIGN_LEFT, CTFJoinTeam1 },
    { NULL,                 CTF_PMENU_ALIGN_LEFT, NULL },
    { "Join Blue Team",     CTF_PMENU_ALIGN_LEFT, CTFJoinTeam2 },
    { NULL,                 CTF_PMENU_ALIGN_LEFT, NULL },
    { "Chase Camera",       CTF_PMENU_ALIGN_LEFT, CTFChaseCam },
    { "Credits",            CTF_PMENU_ALIGN_LEFT, CTFCredits },
    { NULL,                 CTF_PMENU_ALIGN_LEFT, NULL },
    { NULL,                 CTF_PMENU_ALIGN_LEFT, NULL },
    { "Use [ and ] to move cursor", CTF_PMENU_ALIGN_LEFT, NULL },
    { "ENTER to select",    CTF_PMENU_ALIGN_LEFT, NULL },
    { "ESC to Exit Menu",   CTF_PMENU_ALIGN_LEFT, NULL },
    { "(TAB to Return)",    CTF_PMENU_ALIGN_LEFT, NULL },
    { "v" CTF_STRING_VERSION,   CTF_PMENU_ALIGN_RIGHT, NULL },
};

ctf_pmenu_t nochasemenu[] = {
    { "*Quake II",          CTF_PMENU_ALIGN_CENTER, NULL },
    { "*ThreeWave Capture the Flag",    CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,                 CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,                 CTF_PMENU_ALIGN_CENTER, NULL },
    { "No one to chase",    CTF_PMENU_ALIGN_LEFT, NULL },
    { NULL,                 CTF_PMENU_ALIGN_CENTER, NULL },
    { "Return to Main Menu", CTF_PMENU_ALIGN_LEFT, CTFReturnToMain }
};
...
void CTFChaseCam(edict_t *ent, ctf_pmenuhnd_t *p)
{
    ...
    ctf_PMenu_Close(ent);
    ctf_PMenu_Open(ent, nochasemenu, -1, sizeof(nochasemenu) / sizeof(ctf_pmenu_t), NULL);
}
...
void CTFRequestMatch(edict_t *ent, ctf_pmenuhnd_t *p)
{
    char text[1024];
...
void CTFShowScores(edict_t *ent, ctf_pmenu_t *p)
{
    ctf_PMenu_Close(ent);
...
{
    ctf_PMenu_Close(ent);
    ctf_PMenu_Open(ent, creditsmenu, -1, sizeof(creditsmenu) / sizeof(ctf_pmenu_t), NULL);
}
...
bool CTFStartClient(edict_t *ent)
{
    ...
    // A bot is always forced onto a team, because the alternative
...
void CTFObserver(edict_t *ent)
{
    ...
    // start as 'observer'
    //
    // The port lost this guard's braces, message and return, so the test covered
    // only the grapple reset -- and covered it for the client that was *already*
    // an observer rather than the one becoming one, leaving a player who types
    // `observer` mid-swing winched around the map.  One fix was to drop the
    // guard; q2pro fixed it by restoring what the 1.52 release
    // actually had, which is this, and that gives it the decision.
    ...
    ent->client->ps.gunindex = 0;
    ent->client->resp.score = 0;
    // `observer` stays THREEWAVE's verb under ctf -- it drops the
...
void CTFAdmin_UpdateSettings(edict_t *ent, ctf_pmenuhnd_t *setmenu);
void CTFOpenAdminMenu(edict_t *ent);

void CTFAdmin_SettingsApply(edict_t *ent, ctf_pmenuhnd_t *p)
{
...
void CTFAdmin_SettingsCancel(edict_t *ent, ctf_pmenuhnd_t *p)
{
    ctf_PMenu_Close(ent);
    CTFOpenAdminMenu(ent);
}

void CTFAdmin_ChangeMatchLen(edict_t *ent, ctf_pmenuhnd_t *p)
{
    admin_settings_t *settings = p->arg;
...
void CTFAdmin_ChangeMatchSetupLen(edict_t *ent, ctf_pmenuhnd_t *p)
{
    admin_settings_t *settings = p->arg;
...
void CTFAdmin_ChangeMatchStartLen(edict_t *ent, ctf_pmenuhnd_t *p)
{
    admin_settings_t *settings = p->arg;
...
void CTFAdmin_ChangeWeapStay(edict_t *ent, ctf_pmenuhnd_t *p)
{
    admin_settings_t *settings = p->arg;
...
void CTFAdmin_ChangeInstantItems(edict_t *ent, ctf_pmenuhnd_t *p)
{
    admin_settings_t *settings = p->arg;
...
void CTFAdmin_ChangeQuadDrop(edict_t *ent, ctf_pmenuhnd_t *p)
{
    admin_settings_t *settings = p->arg;
...
void CTFAdmin_ChangeInstantWeap(edict_t *ent, ctf_pmenuhnd_t *p)
{
    admin_settings_t *settings = p->arg;
...
void CTFAdmin_ChangeMatchLock(edict_t *ent, ctf_pmenuhnd_t *p)
{
    admin_settings_t *settings = p->arg;
...
ctf_pmenu_t def_setmenu[] = {
    { "*Settings Menu", CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,             CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,             CTF_PMENU_ALIGN_LEFT, NULL },   //int matchlen;
    { NULL,             CTF_PMENU_ALIGN_LEFT, NULL },   //int matchsetuplen;
    { NULL,             CTF_PMENU_ALIGN_LEFT, NULL },   //int matchstartlen;
    { NULL,             CTF_PMENU_ALIGN_LEFT, NULL },   //bool weaponsstay;
    { NULL,             CTF_PMENU_ALIGN_LEFT, NULL },   //bool instantitems;
    { NULL,             CTF_PMENU_ALIGN_LEFT, NULL },   //bool quaddrop;
    { NULL,             CTF_PMENU_ALIGN_LEFT, NULL },   //bool instantweap;
    { NULL,             CTF_PMENU_ALIGN_LEFT, NULL },   //bool matchlock;
    { NULL,             CTF_PMENU_ALIGN_LEFT, NULL },
    { "Apply",          CTF_PMENU_ALIGN_LEFT, CTFAdmin_SettingsApply },
    { "Cancel",         CTF_PMENU_ALIGN_LEFT, CTFAdmin_SettingsCancel }
};

void CTFAdmin_Settings(edict_t *ent, ctf_pmenuhnd_t *p)
{
    ...
    menu = ctf_PMenu_Open(ent, def_setmenu, -1, sizeof(def_setmenu) / sizeof(ctf_pmenu_t), settings);
    CTFAdmin_UpdateSettings(ent, menu);
}

void CTFAdmin_MatchSet(edict_t *ent, ctf_pmenuhnd_t *p)
{
    ctf_PMenu_Close(ent);
...
void CTFAdmin_MatchMode(edict_t *ent, ctf_pmenuhnd_t *p)
{
    ctf_PMenu_Close(ent);
...
void CTFAdmin_Reset(edict_t *ent, ctf_pmenuhnd_t *p)
{
    ctf_PMenu_Close(ent);
...
void CTFAdmin_Cancel(edict_t *ent, ctf_pmenuhnd_t *p)
{
    ctf_PMenu_Close(ent);
}

ctf_pmenu_t adminmenu[] = {
    { "*Administration Menu",   CTF_PMENU_ALIGN_CENTER, NULL },
    { NULL,                     CTF_PMENU_ALIGN_CENTER, NULL }, // blank
    { "Settings",               CTF_PMENU_ALIGN_LEFT, CTFAdmin_Settings },
    { NULL,                     CTF_PMENU_ALIGN_LEFT, NULL },
    { NULL,                     CTF_PMENU_ALIGN_LEFT, NULL },
    { "Cancel",                 CTF_PMENU_ALIGN_LEFT, CTFAdmin_Cancel },
    { NULL,                     CTF_PMENU_ALIGN_CENTER, NULL },
};
...
//  if (ent->client->ctf_menu)
//      ctf_PMenu_Close(ent->client->ctf_menu);

    ctf_PMenu_Open(ent, adminmenu, -1, sizeof(adminmenu) / sizeof(ctf_pmenu_t), NULL);
}
...
void CTFWarp(edict_t *ent)
{
    ...
    // G_CopyString, not strdup: the frees below are gi.TagFree, and the pair has
    // to match.  An earlier pass here converted the frees to the game import and
    // left the allocation on libc, which is undefined behaviour on the first
    // `warp` -- introduced by the merge, not inherited.
...
/*--------------------------------------------------------------------------*/
//
// The ctf row of the ruleset dispatch.  Four rows filled, two inherited.
// Everything else CTF changes about a shared function is a gated inline test
// at the site: a hook is for where a ruleset *replaces* the implementation,
// and these four are the places it does.
//
...
static bool ctf_SelectSpawnPoint(edict_t *ent, vec3_t origin, vec3_t angles)
{
    ...
    if (!spot) {
        ...
        // Falling back is better than gi.error: the boot matrix runs ctf
    ...
    // Threewave never refuses a spot: the refusal is tourney's alone, and
...
// because SetCTFStats blinks the winning team's header from those totals.
static void ctf_BeginIntermission(edict_t *targ)
{
    CTFCalcScores();
    BeginIntermission(targ);
}

const ruleset_ops_t ops_ctf = {
    .name              = "ctf",
    .CheckRules        = ctf_CheckRules,
    .EndLevel          = NULL,      // EndDMLevel, plus level.forcemap
    .ScoreboardMessage = CTFScoreboardMessage,
    .BeginIntermission = ctf_BeginIntermission,
    .SelectSpawnPoint  = ctf_SelectSpawnPoint,
    .ClientPlaced      = NULL,      // CTFStartClient runs before placement
};
