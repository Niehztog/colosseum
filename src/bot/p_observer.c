...
void Cam_Message(edict_t *ent, edict_t *target, char *prefix)
{
    ...
    Q_snprintf(score_buf, sizeof(score_buf), "%d", target->client->resp.score);

    if (target->client->resp.score != 1 && target->client->resp.score != -11)
        Q_strlcpy(fragword, "frags", sizeof(fragword));
    else
        Q_strlcpy(fragword, "frag", sizeof(fragword));
...
//===========================================================================
bool Cam_SpotVisible(edict_t *ent, vec3_t point)
{
...
//===========================================================================
bool Cam_EntityVisible(edict_t *ent, edict_t *target)
{
    vec3_t start, end, sav_origin;
    edict_t *passent;
    edict_t *targent;
...
bool Cam_SpotVisible(edict_t *ent, vec3_t point);                            /* sub_10078125 */
bool Cam_EntityVisible(edict_t *ent, edict_t *target);                       /* sub_100782ad */
...
bool Cam_GetFlyBySpot(edict_t *ent, edict_t *target, vec3_t out)
{
...
void ClientToggleObserver(edict_t *ent)
{
    if (ent->flags & FL_OBSERVER) {
        ...
        //
        // `G_Ruleset()`, not `ctf->value`: gates.py fails the build on a
        ...
        // The donor's `ra->value` arm is unreachable here for a better reason
        // than a cvar: under `arena` this file is never asked, because RA2's
        // own four observer modes are that ruleset's implementation.  Left as
        // an assertion rather than deleted, so a future dispatch mistake is
        ...
        // `pers.weapon` can be NULL here and the donor dereferenced it anyway.
        // PutClientInServer only calls InitClientPersistant on a fresh spawn --
        // under sp and coop it keeps the inventory, and a client that entered
        // observer mode before ever holding a weapon comes back out with none.
        // PutClientInServer has already set gunindex correctly in that case, so
...
//===========================================================================
bool Cam_GetFlyBySpot(edict_t *ent, edict_t *target, vec3_t out_pos); /* sub_10078f4a */
