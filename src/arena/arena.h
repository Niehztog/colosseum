...
// map, so the size has a name and the write is bounded.
...
// the number is the union's, the concept is one.
...
// the eight RA2 kept in its own g_local.h.

// The block add_to_team() copies a name into.  It was a bare 100 at the
// allocation and an unbounded sprintf at the fill.
...
// here -- lives in src/ctf/g_ctf.h.  RA2 declares the same enum and carries
// CTFResetGrapple verbatim, which is what measuring settled it: there is one
// grapple implementation, Threewave's, and RA2's is a fork of it rather than a
// rival design.
...
typedef struct arena_settings_s {
    ...
    int     scorebydamage;
    int     changed;
    // Appended after `changed`, and that is the point.  Every index
    ...
    // The mission packs' five ammo types, appended for the same reason
    // the bot pair were -- everything above is addressed by index from
...
typedef struct arena_s {
    ...
    int     scorebydamage;
    int     changed;
    int     bots;               // mirrors arena_settings_t, index 42
    int     allow_voting_bots;  // mirrors arena_settings_t, index 43
    // Mirrors arena_settings_t, indices 44..48.  The extent assert below
    ...
    int     allow_voting_packweapons;   // index 49
    float       proposetime;
...
// Re-expressed against RA2's queue rather than Gladiator's own arena.
extern  cvar_t      *ra_playercycle;
extern  cvar_t      *ra_botcycle;
// The bot count follows the arena's own size instead of the server's.
...
// pack's weapons, and that is a question about the layer rather
...
// through.
//
// The pack half had no default at all -- an arena.cfg that named no pack
...
// The `arena` userinfo key a bot the fill is adding should carry, or 0
