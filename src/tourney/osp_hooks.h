...
// the ruleset gates actually name, and nothing else.  osp_types.h includes it,
...
// The four modes of play are four rulesets -- `dm`, `dmpro`, `tdm`,
...
// `manual_map` distinguishes a map an operator typed from one the rotation
// chose.  The two intermission timers beside it are in g_local.h: `arena`
// reads them too since R-RA-10, so they are not tourney's to declare.
extern  int     manual_map;
// What tourney does to each client when the level ends -- the demo it
...
// resp.osp_entered's five states.  This is not baseq2's `resp.entered` bool,
// and the two share a name in the merged struct.
//
...
// `ENTERED_QUEUED 3` stood here and is not one of them: no donor site writes
// or reads a 3, and the four states this comment claimed were three plus an
// invention.  The literals still appear as literals at most sites, which is
// the donor's own text; these names are for the sites that were written with
// them.
...
// one.  Read by p_weapon.c's Think_Weapon.
extern  cvar_t  *client_fastweap;
extern  int      bot_watch;

// The Standard Log's four spine-facing writers.  They take the game
...
// The delegated client-command dispatcher.  True when it handled it.
...
// PutClientInServer answers with a frozen, bodiless client that the respawn
// trigger retries.
bool     OSP_spawnRefused(struct edict_s *ent, struct edict_s *spot);
// The two reads the import dropped.  Both answer with the donor's fixed
...
// A powerup freed in the world: logs it if its own timer ran out.
...
// The bot half: a bot has no key to press, so entering the game and
...
// The ping / inactivity / framerate rules.  True means the client is
...
// The observer input, which ClientThink reaches: the autocam (true when it
...
// The two statistics logs, at the spine sites that feed them.
