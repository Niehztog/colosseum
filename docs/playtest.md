...
| `scenarios/ospfixes` | The battery for the tourney fixes, run against BOTH engine families from one binary: half the rows are only interesting if the repair also holds on an engine that predates Q2PRO's game ABI. `tools/playtest.sh -s ospfixes` runs the q2pro arm on retail baseq2 with nothing added -- every row writes the configs it needs; the other arm is `-engine yq2 -bin <q2ded>` against an `API=old` library |
...
| `scenarios/ospimage` | The five answers R-OSP-1 takes from the reconstruction where it is byte-identical to the 1999 image -- `highscores` during play, a table kept by the FFA modes only, a warmup death that is not logged, a duel board with one empty seat, the match-state line blanked at the intermission -- plus the menu flush an autocam observer gets, and the same flush serving Threewave's engine. Each row is a difference that fails on a library without its change, except the ctf row, which is the guard that moving the flush left ctf its own; the two menu rows are timed, so each takes three samples and judges the worst |
...
| `scenarios/ospbotvote` | Can the people playing vote the FILL's bots out (R-OSP-16)? `ra2botvote`'s question one ruleset family over, and the answer was no in two ways at once: the cap counted the bots a VOTE had added, which is zero wherever `botfill` or `bots_minplayers` seated them, and what a vote did remove the fill seated again within 32 frames. Four phases -- the console form and that the count HOLDS where the vote left it, down to none; the bot menu's own "Remove random bots" row stepping off zero, with `vote_enable_bots 0` as its control; the flat-count arm, which takes the cut somewhere else in the source; and the gate refusing by name. Needs $GLADDIR and a q2dm1 mesh, and dies without one rather than skipping |
...
| `scenarios/ra2nullattacker` | Does a damage event that carries **no attacker** take the server down (R-SEC-10)? Nothing assigns `activator` on a `func_door` and a `func_clock` that is not START_OFF never has one, so both hand a NULL to `G_UseTargets` and on to a `target_explosion`'s radius damage. The producer is written into the map's own entity string and re-served through `map_override_path`, so no BSP is edited: a `func_clock` firing `target_explosion`s every few seconds. Two phases for the two reads the NULL reaches -- `message` needs no clients at all (an empty server dies by itself) and `damage` asserts a fighter actually LOST HEALTH, because an explosion that reached nobody proves nothing about the read it never performed. The `message` phase is the behavioural check on `G_UseTargets`' activator test; the static half, over every `use` callback, is `tools/nullattacker.py` |
