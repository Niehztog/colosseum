...
* `server/` is the server side and is NOT the game library: the Docker image that builds the engine and q2admin, the carried patches with `enginepatch.sh`, and the deploy and navigation-mesh tooling. It is the general half of a `q2server-dockerfile` repository merged in on 2026-09-18, and it stays general -- no deployment's hostnames, ports, paths or passwords enter it. Both upstreams it builds are the real ones -- `q2pro/q2pro` and `packetflinger/q2admin`, neither forked -- so a divergence is a patch under `server/<upstream>/` naming the upstream and commit it applies to, and bumping `Q2PRO_COMMIT` or `Q2ADMIN_COMMIT` means refreshing that upstream's patches in the same commit, because a patch that no longer applies fails the build rather than being skipped. A fork instead of a patch is what this already tried: the pinned SHA stopped being reachable when somebody rebased the branch, and the build could not clone it.
