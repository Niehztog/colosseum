...
# The game host, as ssh would take it -- this script runs on the WORKSTATION
# and ships its output there, so there is nothing sensible to default it to.
HOST="${HOST:?set HOST to the game host, e.g. HOST=user@192.0.2.10}"
SERVER_ROOT="${SERVER_ROOT:-$HOME/quake2-colosseum-server}"   # on the HOST
...
TEST = os.environ.get('SERVER_ROOT') or os.path.expanduser('~/quake2-colosseum-server')
def rotation(gd):
    if gd == 'arena':
...
for gd in $GAMEDIRS; do
    echo "      ssh $HOST './make-colosseum-aas.sh $gd'   # from wherever these scripts live there"
done
