#!/bin/bash
#
# setup-colosseum-serverdata.sh -- build a self-contained Colosseum gamedir
# tree to feed a deployment from.
#
#   $SERVER_ROOT/arena/    g_ruleset arena
#   $SERVER_ROOT/xatrix/   g_ruleset dm + xatrix 1
#
# WHAT THE TREE IS FOR.  It is where the .aas navigation meshes are computed and
# kept: scripts/make-colosseum-aas.sh builds them in it and
# scripts/deploy-colosseum-live.sh copies them out into the gamedirs a server
# runs from.  Its shape is a non-public test server's, because standing one of
# those up is what it was written for -- one new variable at a time, against
# copied data, before anything reaches a server people are on.
#
# THE GAMEDIRS KEEP THE NAMES THEY ALREADY HAVE -- `arena` and `xatrix` here,
# NOT the `colosseum` that colosseum's own install instructions assume --
# because a migration has to.  The gamedir name is not private to the server:
...
#
# So they keep the names and get their OWN BASEDIR instead: $SERVER_ROOT is a
# self-contained tree with its own baseq2, and $QUAKE2_DIR is not mounted into
# these containers at all.  That is stronger isolation than a differently-named
# gamedir would have given -- a bug in an unreleased game library cannot reach
# the data a real server runs from, because it cannot see it.
#
...
# a symlink into $QUAKE2_DIR would mean a change to a running server's data
...
#
# THE PAK NUMBERING IS COLOSSEUM'S, NOT THE SOURCE GAMEDIRS'.  Colosseum reads
...
# into this tree's own baseq2, since the source one is not mounted.
#
...
# from baseq2's own paks.  So neither the loose .bsp under $QUAKE2_DIR/arena
# nor those under $QUAKE2_DIR/baseq2/maps have to be copied.
#
...
#
# NO q2admin.  A q2admin deployment loads q2admin as gamei386.so and the real
# mod as gamei386.real.so behind it; these gamedirs load colosseum DIRECTLY.
# That is deliberate for a first test -- one new variable, not two -- but it
# does mean these servers have no anticheat, no cvarban/cmdban, and no
# q2admin-cloud link, so whether colosseum runs correctly UNDER q2admin is a
# separate thing to test before any migration.
#
# IDEMPOTENT.  Existing files are left alone; delete a gamedir to rebuild it.

set -euo pipefail

# Where the source data is read FROM (read only - this script never writes
# into it) and where the test tree is built.
QUAKE2_DIR="${QUAKE2_DIR:-$HOME/quake2}"
SERVER_ROOT="${SERVER_ROOT:-$HOME/quake2-colosseum-server}"
COLOSSEUM_SRC="${COLOSSEUM_SRC:-$HOME/projects/colosseum}"
...
# A FRESH RCON PASSWORD PER GAMEDIR, generated here rather than carried in from
# anywhere.  A test server's password leaking must not hand anyone a server
# people are playing on, which is exactly what reusing one across servers does.
...
# RA2's three v2.50 client paks, already sitting in the source arena gamedir as
...
// file a public server execs to set "public 1" and name its master servers --
// is deliberately NOT exec'd anywhere in this file.  The container
...
// The values the server this stands in for uses.
set timelimit 20
set fraglimit 80
...
// amended once upstream (3a7f9c0 -> 3936dbb, same subject), and the
// repositories this tracks rewrite history routinely -- a hash written down
// here goes stale or vanishes. Two revisions of it matter:
...
# The Reckoning's pak0, which the source xatrix gamedir carries under that same
...
// MAP ROTATION: imported from the rotation the server this stands in for
// already has, if there is one -- nothing here writes it.
//
...
echo "    there is no .so to extract by hand the way a q2admin gamedir needs."
echo
