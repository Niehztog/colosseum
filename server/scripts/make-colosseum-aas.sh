...
# a compose file.  What the reachability pass needs is far smaller than a
# service: one container, on the loopback interface, for the length of the run.
# A long-lived compose service was the earlier shape, and the two defaults below
# are what is left of it.
# LOOPBACK, because the throwaway server below publishes its port there and
# nowhere else.  A LAN address was right only while this drove a long-lived
# service that had to be reachable on one.
