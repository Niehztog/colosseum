...
#
# PREFIX TOLERANCE.  q2pro's `console_prefix` puts a configurable strftime
# prefix on EVERY stdout line, so with it set the engine emits
#     [2026-09-21 21:23:51] Rcon from <ip>:<port>:
#     [2026-09-21 21:23:51] status
# and every pattern below -- all of which are start-anchored or exact -- stops
# matching at once.  The failure is silent and total: WallFly's polls, the
# timer countdowns and the zero spawn-count lines all come back, and the script
# still exits 0.  The same shape killed Dockerfile-daikatana's rcon grep, where
# `timestamp 2` supplied the prefix instead.
#
# So match on a prefix-STRIPPED copy and print the ORIGINAL line, which keeps
# the timestamp on everything that survives.  The strip requires a DIGIT after
# the bracket, so an ordinary `[Clan] Player: ...` chat line is left alone; a
# clan tag that starts with a digit could still be stripped, but only the
# matching copy is affected and the line is printed unchanged either way.
# Written with ${var#pattern} rather than sed or a command substitution so it
# stays fork-free per line -- this runs on every line the server ever prints.
