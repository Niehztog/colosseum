...
| **Amendment 1.50** | **R-197 closes the remaining confirmed non-policy gaps in the five-donor semantic pass.** CTF now publishes only its still-enforced normal `password` bit, never the inert generic `spectator_password` bit; `needpass` is recomputed after a latched ruleset transition (a passworded DM-to-CTF restart reads `2 -> 0`). Its join-menu observer consumes ATTACK and skips inherited jump chase acquisition, leaving the CTF menu as the explicit chase route. **Xatrix and Ground Zero now compose by the spawn-time entity latch.** A non-Rogue Chick uses the base/Xatrix direct eye shot at speed 500, an Xatrix Heat Chick selects `xatrix_monster_fire_heat` while skinned as Heat, and Rogue alone owns blindfire, leading, foot targeting, trace/retry behavior, blocked handling, duck cleanup, and speed/re-fire scaling. The Xatrix Infantry table restores timer 101, fire 103, cock 109; Xatrix Brain and Chick retain their Nightmare pain moves, while Brain's forced duck cleanup is Rogue-only. **RA2 once again refuses a fresh `spectator 1` with its exact donor text; later userinfo changes pass through native Arena placement and its `fightstate` observer.** Its six-message counter is restored after `FloodProtect()`, where the donor puts it. The retained `pers.spectator`/`resp.spectator` mismatch and the flood limiter's ability to shadow the sixth message are both pinned RA2 behavior, not repair candidates. OSP's `m_mode` replacement and its documented deployment/heuristic exclusions remain decisions. The authoritative current sweep is 3,197 donor-touched records, zero unattributed findings, two documented exempt identifiers, and only withdrawn R-195.1 in `NOT_MECHANISED`; its Heat-call prefix is explicitly attributed. Evidence: the CTF/RA2 raw wire probe, `ctfgrapple`, `ctfidview`, `ctfteams`, `ra2queuefire`, `ra2prefire`, `ra2spawn`, `ra2observer`, the 28-row boot matrix, `refinery`/`xcompnd2` reaching frame 152 with Xatrix content, and native `-Werror` builds. Changed: R-197, `doc/donor-fdiff.md`, `doc/regression.md`, `tools/fnsweep.py`, `tools/audit.py`, and the CTF/Xatrix/RA2 source paths above. |
| **Amendment 1.51** | **A review of Amendment 1.50 against the pinned bundles, and the two defects it found in R-197's own port.** R-197 added 82 content-latch tests across 16 monster files -- 45 gating a single Ground Zero-only statement, the rest selecting between arms, with the spine's arm written out in fourteen places -- and all of them, plus the commit before it, were compared function by function against `5d180cb`/`2829dba`/`2ebed93`. Two were wrong. **`hover_pain`'s two non-Rogue arms were handed each other's (sound, move) pair** -- the spine pairs `sound_pain1`/`pain3`, `sound_pain2`/`pain2` and `sound_pain1`/`pain1`, and Ground Zero replaced only the third, so a base or Reckoning Icarus took the 28-frame stagger for a graze and the 12-frame one for a rocket with the wrong voice on both. **Nothing in the tree could see it**: every symbol is the donor's and already in the file, no call and no line count moves, so `fnsweep.py` reads a carried line and `donorgate.py` a correct gate (`doc/donor-fdiff.md` sec 6 item 1, "an argument reordered", arriving across a pair of arms); the boot matrix cannot reach a pain handler, which needs a player to hurt the monster, and the play-test harness cannot drive a monster fight at all. The evidence is the decision table extracted from the function and from the spine and compared as a list, and that is recorded as the whole of it -- no runtime claim is made. **And `xatrix_` was the wrong prefix for six tables and one function.** `m_boss2.c` and `m_gunner.c` are byte-identical between the spine and `port_xatrix`, so R-197's split arms there are the SPINE's, which The Reckoning inherits unchanged; `bq2_` is the file's own name for that and `bq2_gunner_frames_attack_grenade` already sat beside `xatrix_GunnerGrenade` calling it. Renamed to `bq2_boss2_*` and `bq2_GunnerGrenade` with the byte-identity recorded beside each. `xatrix_infantry_*` and `xatrix_monster_fire_heat` keep their prefix -- `m_infantry.c` genuinely differs and that table genuinely is The Reckoning's. `g_ptrs.c` regenerated, not edited (R-SAVE-2). Checks: `make check` 39 audits clean including `check-ptrs` and `arenaspawn`'s six mutation controls, native `-Werror` clean, the 28-row boot matrix 28/28, `ra2waitroom` 8/8 and `ra2twins` 3/3 as unchanged-behaviour controls. Changed: `src/m_hover.c`, `src/m_boss2.c`, `src/m_gunner.c`, generated `src/g_ptrs.c`, `doc/reconciliation.md` R-198, `doc/donor-fdiff.md` sec 6 item 1. |
| **Amendment 1.52** | **R-199 closes R-198's two open medic rows: the attack decision and the patient search are id's under `rogue 0`.** **The attack decision.** id's `medic_checkattack` is two lines -- with `AI_MEDIC` set, call `medic_attack` and return true. Ground Zero replaced it with a different design (a `MEDIC_TRY_TIME` deadline, the target gibbed on expiry, an `AS_STRAIGHT` walk-in past `MEDIC_MAX_HEAL_DISTANCE`) and the tree ran all of it under `rogue 0`, where `timestamp` is a field id never had. The `!enemy || !inuse` guard is kept for BOTH flavours because id has none and `medic_cable_attack` dereferences `self->enemy` on its first line; `abortHeal` stays behind the latch, there being no baseq2 bookkeeping to unwind. **The search, and why it could not be split alone.** The pre-fix state was internally consistent -- callers wrote `monsterinfo.healer` and Ground Zero's search read it -- and simply not id's policy: a base medic searched 400 units instead of 1024 while standing its ground, refused a corpse inside 32, accepted one whose `nextthink` is the fly timer, and consulted `badMedic1`/`badMedic2`. **id's predicate is `if (ent->owner) continue`, so restoring the search drags the claim field with it** -- Ground Zero moved the claim off `owner` on purpose, as its own comment in this file says. So `bq2_medic_FindDeadMonster` (id's body, 28 lines, compared line for line against `5d180cb` and identical), `medic_FindPatient`, `medic_ClaimPatient`, the three claim sites, id's missing `!oldenemy` guard in `medic_idle`, Ground Zero's `monster_done_dodge` at the head of `medic_run`, and the release -- the half that would otherwise have leaked, since Ground Zero releases in the shared `Killed()` and nothing releases `owner`: id's `medic_die` line is restored and `cleanupHeal` and the successful revive release it too, guarded by `owner == self` rather than by the latch, which is stricter than id. **Deliberately NOT split, and named in the file:** `medic_cable_attack`, `medic_hook_launch` and `medic_hook_retract` stay Ground Zero's for both, driven on id's frame distances by `bq2_medic_move_attackCable`; splitting those would be a second medic rather than a latch. Checks: the search body identical to the donor line for line, `make check` 39 audits clean, native `-Werror` clean, the 28-row boot matrix 28/28, `tools/smoke.sh` including the cross-process savegame round-trip. **No runtime check, the second row running to say so:** a heal needs a corpse and a fight and the harness can drive neither, so the donor comparison is the whole of the evidence and `medic_checkattack`'s four branches were traced by hand and recorded as a trace. clang's `-Wunreachable-code` on a latch-folded copy answers only for the one line that changed meaning -- it suppresses diagnostics behind trivially-constant conditions. Changed: `src/m_medic.c`, `doc/reconciliation.md` R-199. |
| **Amendment 1.53** | **R-200 settles which prior art decides a three-donor disagreement, and it is not the rerelease.** The 2023 rerelease was examined first, as id's own merge of these packs, and is REFUSED on its own evidence -- its README: *"In cases of conflicting spawnflags, maps were modified in order to resolve issues, so original expansion pack maps may not load correctly with this DLL."* It ships its own maps; this tree loads the retail ones. So it carries no content latch (its one content switch is `level.is_n64`, from the map path prefix), keeps not a single parallel donor animation table, and resolves `trigger_push`'s spawnflag collision by RENUMBERING `START_OFF` `0x02` -> `0x08`, which R-27 cannot do. Its per-site winners are therefore not corrections to ours. **`gladq2_src/` is the authority instead** -- Mr. Elusive's Gladiator Bot game source is the one merge of these three packs built under our constraint (one DLL, retail maps, `#define XATRIX` and `#define ROGUE` both on), and its rule is consistent across all nineteen shared monster files: **the spine is the default, each pack's change is an ADDITION wrapped in `#ifdef`, with id's line in the `#else`** -- which is R-CORE-11 one level up, since `if (content_flavour & CONTENT_ROGUE) <pack> else <id>` is the runtime form of `#ifdef ROGUE <pack> #else <id> #endif`. **Of the eighteen shared monster files R-197 latched, Colosseum's non-Rogue arm agrees with what Gladiator ships in every one.** They divide by whether he ships the other arm: 50 `#ifdef` blocks give a per-site oracle for BOTH halves in ten files (hover 14, flyer 8, gunner 8, chick 7+1, brain 3+1, soldier 2+1, supertank 2, infantry/medic/parasite 1 each), while eight files carry id's with no `#ifdef` at all (`m_berserk.c`, `m_boss2.c`, `m_boss31.c`, `m_boss32.c`, `m_float.c`, `m_gladiator.c`, `m_mutant.c`, `m_tank.c`, verified by zero Ground Zero tokens in each) -- so what is unsupported there is not the base arm but whether the Rogue arm should exist, and those latches remain this project's own decision. **It corroborates both of the repairs the rerelease appeared to contradict**: Gladiator's `hover_pain` pairs the light band `sound_pain1`/`pain3` and `sound_pain2`/`pain2` with id's `sound_pain1`/`pain1` in the heavy `#else`, which is R-198 arm for arm; and its medic is id's wholesale -- the `owner` claim at all six sites, no `healer`/`abortHeal`/`timestamp`/`monster_slots`, `medic_checkattack` the two-liner, its lone `#ifdef ROGUE` being only `medic_dodge`'s extra `trace_t *` parameter -- which is R-199's base arm field for field. It also supplies the argument FOR the latch: `gladq2_src/g_spawn.c:412` maps `monster_medic_commander` to id's `SP_monster_medic`, so his Medic Commander degrades to a plain medic, which compile-time selection cannot avoid. One idea is still worth taking from the rerelease and never as a behaviour source: shared helpers (`M_ShouldReactToPain`, `PredictAim`, `M_SlotsLeft`, `M_CalculatePitchToFire`) that take the difference as a parameter and need no latch. Recorded as a versioned `CLAUDE.md` because it is a standing instruction for the next site rather than a finding about one; the untracked workspace copy holds a pointer only. Changed: `CLAUDE.md` (new), `doc/reconciliation.md` R-200. |
| **Amendment 1.54** | **R-201: a content latch was gating a pack-exclusive entity's own weapon, and the entity's skin was the selector all along.** R-197 wrote `ChickFireProjectile` as `if ((content_flavour & CONTENT_XATRIX) && s.skinnum > 1)`; the donor's line is `if (self->s.skinnum > 1)` (`port_xatrix:m_chick.c:437`) and the extra term is wrong twice over. **The skin already is the selector and nothing else can set it**: the heat chick is a SEPARATE ENTITY, `monster_chick_heat` with its own spawn-table row, and `SP_monster_chick_heat` is `SP_monster_chick` plus `s.skinnum = 3` -- every write to a chick's skin in this tree is that one and `chick_pain`'s `= 1`, so `skinnum > 1` is unreachable for id's chick. **And `content_flavour` is not a property of the entity**: R-CORE-11a latches it in `ED_CallSpawn` from `G_LayerEnabled`, so it reports whether the SERVER has a layer on and is identical for everything in the level. Its job is choosing between two donors' versions of a monster both of them have -- `chick_pain`'s nightmare early-out, which base and Ground Zero keep and The Reckoning drops, is that case and keeps its gate. A pack-exclusive entity is not, and the spawn path proves it: nothing is refused when a layer is off, so `monster_gekk`, `monster_widow` and the rest keep working under `xatrix 0` while only the heat chick lost its weapon -- the silent-wrong-answer shape `ED_CallSpawn`'s own comment was written against, arriving inside the mechanism that comment installed. **Gladiator cannot arbitrate this one** and that is recorded rather than left as a gap in R-200's method: `monster_fire_heat` appears nowhere in `gladq2_src/m_chick.c` -- he took Ground Zero's blindfire for the chick and none of The Reckoning's heat-seeker -- so the pinned `port_xatrix` bundle is the whole authority, which is the fallback CLAUDE.md already names. **Not fixed, deliberately:** The Reckoning left `chick_pain`'s `s.skinnum = 1` alone, so a heat chick hurt below half health drops to skin 1 and fires rockets for the rest of its life; the tree that changed that is the rerelease, which R-200 declines as a behaviour source, and the file says so in place. The general rule is now a `CLAUDE.md` bullet: the latch is for a monster both donors have, and where the world already carries the distinction the world's own state is the selector. Checks: the donor line compared directly, `make check` 39 audits clean, native `-Werror` clean, the 28-row boot matrix 28/28; no runtime check, a heat chick having to be shot at and the harness being unable to drive a monster fight. Changed: `src/m_chick.c`, `CLAUDE.md`, `doc/reconciliation.md` R-201. |
| **Amendment 1.55** | **R-202 extends R-201's entity-identity rule to the two Ground Zero aliases that share base monster implementations, and normalizes the related Kamikaze ordering.** `monster_medic_commander` maps to `SP_monster_medic` and `monster_daedalus` maps to `SP_monster_hover`; the spawn path deliberately accepts both classnames with `rogue 0`, while `content_flavour` truthfully remains a server-wide latch. The old layer-only selectors therefore let a Commander become a 300-health base Medic and let Daedalus take base pain, attack, reattack and blocked paths. `medic_UsesRogueBehavior()` and `hover_UsesRogueBehavior()` now delegate to the generic selector that combines the Rogue layer arm with exact live Rogue-exclusive identities, and every family-level selector uses them; ordinary shared monsters retain their original latch behavior. The Commander cable's explicit `if/else` pointer assignments remain because `genptr.py` generates `save_ptrs[]` from that source shape. `flyer_attack()` now tests Kamikaze `mass > 50` before the base Flyer arm, as `port_rogue` does. The expanded `donorgate.py` rejects either missing identity, any bypassing raw selector, or inverted Kamikaze order, with three new mutation controls. Checks: all 39 audits and native debug/release `-Werror` builds clean; `make check-ptrs`, smoke and the 28-row boot matrix clean; retail `rbase1` save/load under both layer states gives each of six Commanders 600/600 health, mass 600, yaw 40, skin 2 and four slots, and the ten Daedaluses that spawn in co-op their donor state and matching callbacks. The headless harness cannot make a monster fight, so pain/attack selection remains mechanically guarded source/donor control-flow evidence rather than a false live-combat claim. Changed: `src/m_medic.c`, `src/m_hover.c`, `src/m_flyer.c`, `tools/donorgate.py`, `doc/reconciliation.md` R-202. |
| **Amendment 1.56** | **R-203 makes the effective-entity rule apply to every conflicting shared base/Xatrix/Rogue behavior, and makes R-MODE-3's Tag narrowing executable.** The source had two silent inversions: under `rogue 0`, `g_ai.c` still ran Ground Zero's shared arm for ordinary base/Xatrix monsters; and `gamerules` could populate or reach Ground Zero's `DMGame` callbacks under the OSP family, although R-MODE-3 says Tag survives only under CTF and Arena. **Gladiator decides the first:** its `#ifdef ROGUE` arm maps to `M_UsesRogueBehavior(ent)`, its `#else` to the base/Xatrix arm. The generic selector is the Rogue layer plus exactly the eight live Rogue-only spawn-table identities -- Stalker, Turret, Daedalus, Carrier, Widow, Widow2, Medic Commander and Kamikaze -- no commented-out or extra classnames. It drives all shared AI attack/target/sight/pursuit decisions, and two serialized sight caches preserve base visibility of disguise while Rogue-effective monsters exclude disguise except for the intentional violator path. The changed level layout advances `SAVE_VERSION` from `0x100` / `8` to `0x101` / `9`, refusing old saves rather than misreading them. The same target-selected split restores base Medic `owner` claims and retaliation exclusions while leaving Tesla and spawned-minion effects state-driven; barrels and dead soldiers now choose their direct donor arm. **R-MODE-3's global half uses a different selector:** `G_UsesRogueGameRules()` is nonzero `gamerules` AND CTF/Arena, and gates every `DMGame` installation and invocation, so it cannot run beside OSP's own protocol. `donorgate.py` checks the exact identities, both AI arms/caches, save descriptor/version, combat/map-entity arms, and all DMGame consumers with adversarial controls. Changed: `src/g_ai.c`, `src/g_combat.c`, `src/g_misc.c`, `src/g_monster.c`, `src/g_local.h`, `src/g_save.c`, `src/g_ruleset.[ch]`, `src/g_main.c`, `src/g_spawn.c`, `src/p_client.c`, `src/p_hud.c`, `src/p_view.c`, `src/rogue/g_newdm.c`, `tools/donorgate.py`, `doc/reconciliation.md` R-203. |
| **Amendment 1.57** | **R-204 corrects an over-broad reading of R-203's runtime split.** `port_rogue` differs from baseq2 in `ai_turn()` by suppressing yaw under `AI_MANUAL_STEERING`, but Gladiator's merged `g_ai.c` has no `#ifdef ROGUE` at that site and retains id's unconditional `M_ChangeYaw(self)`. Under R-200, absence of the integration fence is a deliberate spine decision, not an omitted donor arm. `ai_turn()` therefore stays unconditional; its only live caller is the base Actor animation family, while Rogue actors that write manual steering use selected `ai_charge`/`ai_run` paths instead. `donorgate.py` now rejects a selector in `ai_turn` and carries a mutation for it. Changed: `src/g_ai.c`, `tools/donorgate.py`, `doc/reconciliation.md` R-204, `doc/regression.md`, `doc/donor-fdiff.md`. |
| **Amendment 1.58** | **R-205 gives Rocket Arena's Arena numbers, parser, and generated teams explicit bounded contracts.** Arena 0 remains the lobby and `1..MAX_ARENAS` are the playable IDs, so storage is `arenas[MAX_ARENAS + 1]`; ID 32 is valid, while an invalid worldspawn count falls back to one id arena and every external ingress (bot userinfo, teleporter target, `AddtoArena`, and grant lookup) rejects an ID outside the configured count before indexing. `arena.cfg` no longer lets an unbounded `fscanf("%s")` write or a bounded replacement silently reinterpret a token tail: raw tokens, accumulated definition values, definition blocks, nesting, and unterminated declarations now fail the complete parse and leave defaults rather than partial state. `tools/bounded.py` consequently extends R-SEC-1/R-VER-30 to raw string/scanset conversions, including zero-width forms. Dynamically allocated team names transfer ownership only after a successful insert and are freed with empty nonlocked teams; a full-team creation menu now keeps the existing failure behavior rather than opening an unusable arena menu. `fnsweep.py` recognizes a struct-returning function before treating its leading `struct` as a type declaration, and attributes the deliberately removed unsafe RA2 scanner to R-SEC-1. `arenaspawn.py`, `bounded.py`, and `fnsweep.py` carry adversarial controls for all new parser/index/extractor boundaries. Changed: `src/arena/{arena.[ch],maploop.c,ra2menus.c}`, `src/bot/bl_spawn.c`, `src/g_misc.c`, `tools/{arenaspawn.py,audit.py,bounded.py,fnsweep.py}`, `doc/reconciliation.md` R-205, `doc/regression.md`. |
| **Amendment 1.59** | **R-206 makes Tesla liveness a state check before, not inside, the shared AI donor-arm selector.** R-203 deliberately preserves Tesla targeting as a live relationship for any actor, including a base/Xatrix monster under `rogue 0`. Its initial `ai_charge()` split returned through id's arm before Ground Zero's stale-enemy test, so an exploded Tesla left those actors calculating yaw and movement from the cleared edict origin. The liveness guard now precedes the base/Rogue branch; this is not a new global Rogue behavior but the safety condition needed by the retained cross-arm relationship. `donorgate.py` requires its order and mutates it away. Changed: `src/g_ai.c`, `tools/donorgate.py`, `doc/reconciliation.md` R-206, `doc/regression.md`. |
| **Amendment 1.60** | **R-207 closes the OSP behavioral review and makes the generic hook request an explicit OSP-only baseline overlay.** The OSP donor's delayed intermission board, chase/autocam HUD ownership, observer admission boundary, chat routing, command restrictions, score navigation, friendly-fire precedence, Strength knockback, rail damage, Ammo Pack marker, rune isolation, logical layout slots, filesystem failure paths, and Standard Log base-directory path all now retain their donor behavior or their documented secure replacement. `STAT_OSP_LAYOUT2` intentionally remains unwritten: its only donor producer was the retired ngLog/worldstats stack, not a missing OSP HUD path. Under the four OSP rulesets only, latched `hook 1` turns native `hook_enable` on after an OSP baseline is installed, including after a voted/default configuration `exec`; `hook 0` does not force it off, and live hook votes/carryover retain native authority. The bridge, feature advertisement, botlib, and diagnostics all use OSP's integer `hook_enable` semantics. CTF and Arena keep `ctf_hook` and `allow_grapple` respectively. `donorgate.py` now enforces the ordering, scope, integer predicates, and adversarial controls. Changed: shared client/HUD/combat/item/weapon code, `src/tourney/{osp_cmds.c,osp_config.c,osp_display.c,osp_hooks.h,osp_main.c,osp_maps.c,osp_players.c,osp_runes.c,sl_write.c}`, `src/{g_ruleset.[ch],g_main.c,g_spawn.c}`, `src/bot/bl_main.c`, `tools/{donorgate.py,slotkind.py}`, `doc/reconciliation.md` R-207, `doc/regression.md`, `doc/cvars.md`. |
| **Amendment 1.61** | **R-208 preserves content flavor through Jorg's direct delayed Makron spawn.** `MakronToss()` uses `G_Spawn()` and reaches `SP_monster_makron()` directly, so it bypasses `ED_CallSpawn()`'s ordinary layer latch; copying the parent's `content_flavour` before its delayed callback keeps one boss encounter on one resolved base/Rogue behavior arm, including Rogue Makron `AI_IGNORE_SHOTS`. The complete dynamic-spawn inventory found no second affected actor path: normal reinforcement/minion creation and resurrection use `ED_CallSpawn()`, while the remaining direct spawns are wrappers, probes, projectiles, or non-AI entities. `donorgate.py` enforces both the direct inheritance ordering and the shared helper latch. Changed: `src/m_boss32.c`, `tools/donorgate.py`, `doc/reconciliation.md` R-208, `doc/regression.md`, `doc/donor-fdiff.md`. |
| **Amendment 1.62** | **Final review closes one Arena ownership and one OSP configuration-lifecycle defect.** A stale RA2 team-menu row deliberately re-creates a deleted team, but its text belongs to the menu and is freed when the callback closes it; `menuAddtoTeam()` now copies that label and transfers the copy only when `add_to_team()` creates the replacement, freeing it for an existing team or a rejection. A voted/default OSP configuration `exec` now re-synchronizes the capped `rune_stat` cache from its effective `runes_enable` after the engine executes the config and before feature publication or rune scheduling; ordinary map transitions retain live rune-vote/carryover state. `arenaspawn.py` and `donorgate.py` carry adversarial controls for both lifecycles. Changed: `src/arena/{arena.c,ra2menus.c}`, `src/{g_ruleset.c,g_spawn.c}`, `src/tourney/{osp_hooks.h,osp_main.c}`, `tools/{arenaspawn.py,donorgate.py}`, `doc/{reconciliation.md,regression.md,donor-fdiff.md,cvars.md}` R-209/R-210. |
| **Amendment 1.63** | **R-211 makes one asleep control able to fail, and closes four sites the R-202..R-210 sweep opened or left half-gated.** R-204's `ai_turn` check fired only on the token `M_UsesRogueBehavior`, so `port_rogue`'s own `AI_MANUAL_STEERING` guard -- the arm R-204 exists to reject -- could be restored with the audit silent; verified, then replaced with a structural test for any condition between the `FindTarget` return and the yaw, plus both mutant shapes. R-203's `FindTarget` split had restored baseq2's bare `client->owner->flags` deref, but Gladiator's NULL guard is OUTSIDE its ROGUE fence (`gladq2_src/g_ai.c:597`), so under R-200 it belongs to both arms and the split could only differ by crashing; the guard is unconditional again and the audit rejects any unguarded read beside it. `MakronSpawn` is the only other reader of the cache R-203 redefined and now picks by the Makron's own flavor rather than seeing a disguised player through the base cache. The OSP rune spawner takes the donor's integer test (`port_osp:g_spawn.c:761`), since `rune_stat` is derived with the same cast and a fractional `runes_enable` otherwise schedules a spawner with no rune enabled -- R-210's own described failure, and R-207's stated rule for `hook_enable`. And R-203's DMGame gate was reachable around: Tag's token enters through the SPAWN TABLE and the ITEM LIST, so under OSP with `gamerules 2` it still spawned and could be picked up while every callback that scored or cleaned it up was gone; Deathball is worse, because not calling `InitGameRules()` outside CTF/Arena also removed the `default:` arm that used to reset an invalid `gamerules 3` before a dball map could act on it. Both files get one predicate -- `Tag_Active()` / `DBall_Active()` -- asking R-MODE-3's question beside the mode number, called by all nine world-facing entry points, with a second `gamerules->value` read anywhere else in either file rejected. Checks: every fix verified by reinstating the exact prior text and confirming the audit reports it; six new `donorgate.py` controls, all thirty fire; `make check` clean at `-Werror` with all 39 audits, `check-ptrs` clean, boot matrix 28/28, `smoke.sh` clean including the cross-process savegame round-trip. Changed: `src/g_ai.c`, `src/m_boss32.c`, `src/g_spawn.c`, `src/rogue/{dm_tag.c,dm_ball.c}`, `tools/donorgate.py`, `doc/{reconciliation.md,regression.md}` R-211. |
...
* **R-MODE-3.** `xatrix` and `rogue` are independent latched content layers,
  ...
  spawnable. A layer selects a donor arm only where two donors supplied a
  variant of the same entity; it continues to gate direct Ground Zero world
  behavior such as plat2 and danger-area logic. A pack-exclusive entity keeps
  its own donor behavior when the spawn path admits its classname with that
  layer off: `M_UsesRogueBehavior()` combines the Rogue layer with the exact
  live Rogue-exclusive monster identities for shared AI/combat decisions.
  Relationship state such as Tesla targeting and spawned-minion accounting is
  not an entity-family selector and remains state-driven.

  *One narrowing, recorded in 1.36 and mechanically enforced in 1.56.*
  `gamerules`
  ...
  `CheckDMRules()` by name. `G_UsesRogueGameRules()` is the one predicate at
  every installation and invocation site, so an OSP-family ruleset cannot reach
  the Ground Zero callback table merely because `gamerules` is nonzero. The
  layers stay valid with every ruleset — every Ground Zero item, monster and
  weapon still spawns and behaves — what narrows is this one global DM-rules
  protocol because the ruleset that used to carry it is now somebody else's
  code.
* **R-MODE-4.** Modifiers: `teamplay`, `hook`, `runes`/`techs`, and bots. Each
  ...
  **OSP hook bridge.** In `dm`, `dmpro`, `tdm`, and `duel` only, latched
  `hook 1` is a one-way request to enable OSP's native `hook_enable`: after OSP
  installs or resets a baseline, it writes `1` only when OSP's integer
  interpretation says the hook is disabled. `hook 0` makes no write, so an
  explicit `hook_enable` setting remains authoritative. This is not a live
  lock: a passed `vote hook 0|1` controls the running match, and
  `vote_carryover 1` retains that vote without a reapplication. A fresh
  baseline, including one written by a voted or empty-server default
  configuration `exec`, reapplies the pending request after the exec and before
  that map starts. `G_ModifierEnabled(MOD_HOOK)`, `sv ruleset`, OSP's feature
  advertisement, and botlib all report the same live integer state. CTF and
  Arena do not participate in this bridge: their hook authorities are
  `ctf_hook` and the active Arena's `allow_grapple` setting (R-207).
...
* **R-MODE-7.** Composability matrix — the contract:
  ...
  * The `hook` cells name the native mechanism available in each ruleset; they
  do not make the generic `hook` cvar a cross-ruleset override. Its one-way
  baseline bridge reaches OSP only. CTF continues to read `ctf_hook`, and Arena
  continues to read its parsed `allow_grapple` setting (R-207).
...
* **R-ENG-1a.** *The game ABI is a build switch, and `new` is the default.*
  ...
  *Observed, not reasoned:* `tools/playtest.sh` on the `API=old` release prints
  ...
  `SAVE_VERSION` is `0x101` against `9` after R-203 added a serialized level
  pointer, so a crossed savegame is refused rather than misread.
...
* **R-SEC-1.** No unbounded copy or raw `fscanf` string scan of
  client-controlled data. Every `strcpy`, `strcat`, `sprintf` and
  `strncpy`-without-termination, and every `fscanf` string/scanset conversion
  without a positive field width, on a path reachable from userinfo, a client
  command, a chat string, an address string or a config file is replaced with a
  bounded, always-terminating form.
  ...
  *Enforced in 1.24 as a **ban** rather than as a reachability judgement, because
  ...
  do not appear in `src/` **at all**, and no raw `fscanf` string/scanset
  conversion lacks a positive width. A parser that needs the whole raw token
  must reject overflow instead of interpreting a bounded conversion's tail as a
  second token. 189 copy sites converted, of which three were live defects and
  all three were config-file or userinfo paths this requirement names by hand.
...
* **R-VER-15.** *The original harness's unfinished checks are inherited.*
  **All eight are discharged as of 1.14** — the last two, items 4 and 8, by
  ...
  7. **`SAVE_VERSION` drifted in rogue** — local transforms skipped q2pro's
     ...
     risk.* **Confirmed clean 2026-08-26 (1.27):** the pre-R-203 tree carried
     the same `#if USE_NEW_GAME_API` guard, `0x100` / `8`, and
     `SAVE_MAGIC1`/`SAVE_MAGIC2` as `q2pro/src/game/g_save.c`. R-203 then
     deliberately changes the pair to `0x101` / `9` because
     `level.rogue_sight_client` changes the serialized level layout. That is
     not donor drift: the mismatch rejects an old layout rather than allowing
     `ReadLevel()` to misread every subsequent field. The item stays on this
     list with both the original confirmation and the deliberate format change
     attached, because the risk was real and the answer is now written down
     somewhere (R-128, R-203).
...
* **R-VER-30.** *No unbounded string copy or raw `fscanf` string scan anywhere
  in `src/`* (new in 1.24; expanded in 1.58). `tools/bounded.py` bans `strcpy`,
  `strcat`, `sprintf`, `vsprintf`, `strncpy` and `strncat` outright, and rejects
  `fscanf` string/scanset conversions without a positive width, rather than
  asking which inputs reach them. Reachability is not decidable and R-SEC-1's
  list of reachable paths is a list of *examples*. `src/shared/` is exempt as a
  path, because it is vendored byte-identical from `q2pro/src/shared` and
  `divergence.py` is the check that covers it. Seven controls include a call
  split across two lines, an unbounded raw token, and a zero-width token.
...
* **R-VER-35.** *Every donor's feature set is walked definition by definition*
  ...
  **The finding is narrow and the report is not, and the difference is written
  ...
  missing from its own site is the *normal* case under R-MODE-5, and 1,168 of
  them are elsewhere in this tree verbatim. At R-195 that rule contained two of
  seven findings; the other five turned on identifiers that were all present and
  were missing a condition, a reader, an argument or a call site.
  `NOT_MECHANISED` named the resolver each shape would need. R-196 and R-197
  close those non-policy paths, leaving only R-195.1's withdrawn provenance
  note; a clean run remains not a claim that the donors are fully carried.
